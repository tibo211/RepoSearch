...
final class GitHubDataTests: XCTestCase {
    func testDecoding() throws {
        let data = testResponse.data(using: .utf8)!
        
        let result = try JSONDecoder()
            .decode(SearchRepositoriesResult.self, from: data)
        
        XCTAssertNotNil(result.items.first)
        let repo = result.items.first!        
        
        XCTAssertEqual(repo.name, "Tetris")
        XCTAssertEqual(repo.owner.name, "dtrupenn")
    }
}

/// An example results of a search repositories call found on:
///  https://docs.github.com/en/free-pro-team@latest/rest/search/search?apiVersion=2022-11-28#search-repositories
private let testResponse = """
{
  "total_count": 40,
